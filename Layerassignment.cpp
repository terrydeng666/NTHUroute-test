...
{
    if (global_net_id == 0) // 51187
        std::cout << "klat_xy " << start->x << "," << start->y << "," << cur_idx << " " << end->x << "," << end->y << "," << cur_idx << endl;
...
{
    if (global_net_id == 0) // 51187
        std::cout << "klat_z " << start->x << "," << start->y << "," << cur_idx << " " << start->x << "," << start->y << "," << pre_idx << endl;
...
void update_path_for_klat(Coordinate_2d *start, int start_pin_layer)
{
    ...
    while (!q.empty())
    {
        ...
        {
            std::cout << "-------------\n";
            std::cout << "qqqqqqqqqqqq " << cur->x << ' ' << cur->y << ' ' << cur->z << std::endl;
            std::cout << rr_map->get_net_name(global_net_id) << endl;
        }
        // std::cout << "-------------\n";
        // std::cout << "qqqqqqqqqqqq " << cur->x << ' ' << cur->y << ' ' << cur->z << std::endl;
        // std::cout << rr_map->get_net_name(global_net_id) << endl;
        ...
        for (i = 0; i < 4; ++i)
        {
            ...
            if (path_map[cur->x][cur->y].edge[i] == 1) // check leagal
            {
                ...
                if (path_map[end->x][end->y].val == 2)
                {
                    ...
                    if (global_net_id == 0)
                        std::cout << "q pushhhhhhhhhhh " << end->x << " " << end->y << " " << temp_z << endl;
                ...
                while (1)
                {
                    ...
                    else // legal
                    {
                        ...
                        {
                            if (global_net_id == 0)
                                std::cout << "z direction change" << endl;
                            //  update capacity
                            update_cur_map_for_klat_xy(temp_z, &coor_array[cur->x][cur->y], &coor_array[end->x][end->y]);
                            two_pin_nets.emplace_back(coord_3d_map[cur->x][cur->y][temp_z], coord_3d_map[end->x][end->y][temp_z]);
                            if (global_net_id == 0)
                        ...
                        if (path_map[next->x][next->y].val == 2)
                        {
                            ...
                            if (global_net_id == 0)
                                std::cout << "q pushhhhhhhhhhh " << next->x << " " << next->y << " " << temp_z << endl;
    ...
    {
        std::cout << "name " << rr_map->get_net_name(global_net_id);
    ...
    wire_result.push_back(two_pin_nets);
    via_result.push_back(via);
...
/*bool in_cube_and_have_edge(int x, int y, int z, int dir, int net_id)
{
    ...
    {
        if (cur_map_3d[x][y][z].edge_list[anti_dir[dir]]->used_net.count(net_id))
        {
            return true;
        }
        else
...
void klat(int net_id)
{
    ...
    if (rr_map->get_net_name(net_id) == "axi_req_o[12]")
    {
        std::cout << "pin_num = " << pin_list->size() << std::endl;
        for (int i = 0; i < pin_list->size(); i++)
        {
...
#endif
    // std::cout << "after DP " << rr_map->get_net_name(net_id) << " " << net_id << endl;
    update_path_for_klat(start, start_pin_layer);
    // std::cout << "after update....................................." << endl << endl;
    /*
    #ifdef PRINT_DP_PIN
        if(check_net){
            exit(0);
...
void sort_net_order(void)
{
    ...
    for (times = 0.8; times <= 1.2; times += 0.2)
    {
        ...
        for (i = 0; i < max; ++i)
            net_info[i].xy = 0;
#endif
        std::cout << "temppppppppppp" << temp_buf[2] << " " << temp_buf[3] << std::endl;
        for (i = 0; i < max; ++i) // max
        {
            // std::cout << "max " << max << " i " << i << endl;
            if (temp_net_order[i] == 0)
                cout << "i " << i << endl;
...
{
    int i, j, max = rr_map->get_netNumber();

    // for (i = 0; i < max; ++i)
    // {
    //     fout << rr_map->get_net_name(i) << "\n(\n";
    //     for (const auto &seg : wire_result[i])
    //     {
    //         fout << rectToGuide(seg.first.x, seg.first.y, seg.second.x, seg.second.y, seg.first.z);
    //     }
    //     for (const auto &via : via_result[i])
    //     {
    //         fout << viaToGuide(via.first.x, via.first.y, via.first.z, via.second.z);
    //     }
    //     fout << ")\n";
    // }
...
void Layer_assignment(const std::string &guide_path)
{
    ...
    std::cout << "???????????????" << coord_3d_map[88][384][0].z << endl;
    find_overflow_max();
